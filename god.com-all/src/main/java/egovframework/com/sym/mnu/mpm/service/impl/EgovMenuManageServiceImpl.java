...
public class EgovMenuManageServiceImpl extends EgovAbstractServiceImpl implements EgovMenuManageService {
    ...
    private String bndeRegist(InputStream inputStream) throws Exception {
        ...
        try {
            /*
             * 오류 메세지 정보 message = "99"; //프로그램목록테이블 데이타 존재오류. message = "99"; //메뉴정보테이블 데이타 존재오류. message = "90"; //파일존재하지 않음. message = "91"; //프로그램시트의 cell 개수 오류 message = "92"; //메뉴정보시트의 cell 개수 오류 message = "93"; //엑셀 시트개수 오류 message = "95"; //메뉴정보 입력시 에러 message = "96"; //프로그램목록입력시 에러 message = "0"; //일괄배치처리 완료
             */
        ...
        try {
            ...
            for (int j = 1; j < rows; j++) { // row 루프
                ...
                if (row != null) {
                    ...
                    if (cell != null) {
                        Double doubleCell = new Double(cell.getNumericCellValue());
                        vo.setUpperMenuId(doubleCell.longValue());
                    }
